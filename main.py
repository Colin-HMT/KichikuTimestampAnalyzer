...
def filetrans(file_path, appkey, filelink, word_output):
    ...
    postrequst.set_domain(domain)
    postrequst.set_version(api_version)
    postrequst.set_product(product)
    postrequst.set_action_name("SubmitTask")
    postrequst.set_method('POST')
    ...
    postrequst.add_body_params("Task", task)
    taskid = ""
    try:
        postresponse = client.do_action_with_exception(postrequst)
